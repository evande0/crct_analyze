...
def extract_all_data(folder):
    ...
    logger.info(f"\n✔️ Extracted data from {count} JSON files in '{folder}'\n")
...
def extract_data(filepath):
    ...
    logger.info(f"....Finished extracting data from {os.path.basename(filepath)}.")
    return totals
...
#         logger.debug(f"....Extracted row: {row}")
        rows.append(row)
    logger.info("....Extracted rows from data")
    return rows
...
#         logger.debug(f"....Set totals value for key {key} to {totals[key]}")
    logger.info(f"....Calculated totals for {os.path.basename(filepath)}")
    return totals
...
def append_to_totals_csv(totals):
    ...
    logger.info(f"....Appended scenario totals to {TOTALS_FILENAME}")
